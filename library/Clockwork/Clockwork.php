...
class Clockwork_Clockwork {
  ...
  */
  public function __construct($key, array $options = array()) {
    if (empty($key)) {
      throw new Clockwork_ClockworkException("Key can't be blank");
    } else {
      $this->key = $key;
    }
  ...
  */
  public function send(array $sms) {
    if (!is_array($sms)) {
      throw new Clockwork_ClockworkException("sms parameter must be an array");
    }
    ...
    if (isset($err_no)) {
      throw new Clockwork_ClockworkException($err_desc, $err_no);
    }
